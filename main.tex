...
\begin{table}[t]
    ...
    \resizebox{0.65\columnwidth}{!}{%
        \begin{tabular}{lr}
            ...
            \quad Embedding Layer ($W_{\text{feat2emb}}$) & 600 \\
            \quad Init Query Proj ($W_{\text{Q\_init}}$) & 600 \\
            \quad Init Key Proj ($W_{\text{K\_init}}$) & 600 \\
            \quad Init Value Proj ($W_{\text{V\_init}}$) & 600 \\
...
This compact design makes LIQCA a practical candidate for deep learning applications on edge devices, bridging the gap between large-scale models and extremely resource-limited hardware.
...
Section~\ref{sec:exp_baselines} introduces the baseline methods and the training hyperparameters employed for both the baselines and LIQCA.
...
We perform L1-norm structured pruning using the \textbf{torch-pruning} library together with the Dependency Graph (DepGraph) algorithm \citep{fang2023depgraph}. The L1-norm criterion ranks filters by the magnitude of their weights and removes those with the smallest sums of absolute values, which are assumed to be less informative \citep{filters2016pruning}. DepGraph automatically identifies and groups coupled layers (e.g., within residual connections), ensuring structural integrity and dimensional consistency during pruning. For each baseline, we first train the model for 10 epochs of a 90-epoch schedule to estimate filter importance. We then employ a binary search procedure to determine the global sparsity ratio that matches the target FLOPs or parameter budget and prune the model accordingly, excluding the final classification layer and, for transformer-based architectures, the query–key–value (QKV) projection layers. This early-pruning strategy follows \citet{you2019drawing}, who showed that using roughly 6.5\%--12.5\% of the full training schedule is sufficient to identify high-quality subnetworks. With this protocol, we establish a standardized benchmark that enables a fair evaluation of LIQCA against all baselines under matched resource constraints.
    ...
    \resizebox{1.0\columnwidth}{!}{%
        \begin{tabular}{l l r r r r}
            ...
            \multirow{6}{*}{\textbf{Private}} 
             ...
             & MobileNetV4-S   & 84.07 & 83.23 & \textbf{82.17} & 82.69 \\
             & Xie2019         & 81.71 & 81.05 & 78.98 & 80.00 \\
             & DeiT-Tiny       & 80.53 & 77.25 & \textbf{82.17} & 79.63 \\
             & MobileViT-XXS   & 82.30 & 82.25 & 78.34 & 80.39 \\
             \cmidrule{2-6} 
             & \textbf{Ours}   & \textbf{86.73} & \textbf{88.36} & \textbf{82.17} & \textbf{85.15} \\
            \bottomrule
        \end{tabular}%
    }
\end{table}
...
\begin{table}[t]
    \centering
    \caption{Performance comparison of original (unpruned) models. \textbf{Size (.pth)} and \textbf{Size (.onnx)} denote the physical storage requirements of the PyTorch checkpoint and the converted ONNX file, respectively. Note that the computational complexity and storage requirements of baseline models significantly exceed those of LIQCA.}
    ...
    \resizebox{\columnwidth}{!}{%
        \begin{tabular}{l r r r r r r@{ $\pm$ }l r@{ $\pm$ }l}
            ...
            \textbf{EfficientNet-B0} & 0.83 & 4.01 & 15.57 & 15.29 & 21.08 & 32.30 & 1.89 & 31.03 & 1.21 \\
            \textbf{MobileNetV4-S}   & 0.39 & 2.50 & 9.70  & 9.49  & 1.00  & 7.79  & 0.18 & 128.48 & 2.55 \\
            \textbf{Xie2019}         & 2.87 & 9.16 & 34.95 & 34.95 & 28.73 & 34.49 & 1.67 & 29.04 & 1.07 \\
            \textbf{DeiT-Tiny}       & 2.15 & 5.52 & 21.12 & 21.23 & 2.91  & 34.01 & 1.57 & 29.45 & 1.08 \\
            \textbf{MobileViT-XXS}   & 0.54 & 0.95 & 3.74  & 3.87  & 10.44 & 22.96 & 0.56 & 43.58 & 0.94 \\
            \midrule
            \textbf{Ours}            & 0.18 & 0.03 & 0.16 & 0.19 & 17.31 & 16.48 & 1.60 & 61.07 & 3.94 \\
            \bottomrule
        \end{tabular}%
    }
\end{table}
...
\begin{table}[t]
    \centering
    \caption{Target global sparsity ratios determined via binary search. Note that identical sparsity targets were applied to both L1 Pruning and DepGraph for fair comparison.}
    ...
    \resizebox{0.6\columnwidth}{!}{%
        \begin{tabular}{l r r}
            ...
            \midrule
            \textbf{EfficientNet-B0} & 0.5743 & 0.9368 \\
            \textbf{MobileNetV4-S}   & 0.3234 & 0.9059 \\
            \textbf{Xie2019}         & 0.9214 & 0.9426 \\
            \textbf{DeiT-Tiny}       & 0.8169 & 0.9852 \\
            \textbf{MobileViT-XXS}   & 0.4757 & 0.9088 \\
            \bottomrule
        \end{tabular}%
    }
\end{table}
...
\begin{table}[t]
    ...
    \resizebox{\columnwidth}{!}{%
        \begin{tabular}{l l l r r r r}
            ...
            % Private Dataset (Updated Values)
            \multirow{11}{*}{\textbf{Private}} 
             & \multirow{2}{*}{EfficientNet-B0} & L1 pruning & 82.60 & 85.51 & 75.16 & 80.00 \\
             & & DepGraph & 82.60 & 78.16 & \textbf{86.62} & 82.18 \\
             \cmidrule{2-7}
             & \multirow{2}{*}{MobileNetV4-S} & L1 pruning & 83.19 & 84.25 & 78.34 & 81.19 \\
             & & DepGraph & 81.42 & 81.33 & 77.71 & 79.48 \\
             \cmidrule{2-7}
             & \multirow{2}{*}{Xie2019} & L1 pruning & 81.12 & 78.18 & 82.17 & 80.12 \\
             & & DepGraph & 82.01 & 82.00 & 78.34 & 80.13 \\
             \cmidrule{2-7}
             & \multirow{2}{*}{DeiT-Tiny} & L1 pruning & 80.83 & 77.71 & 82.17 & 79.88 \\
             & & DepGraph & 80.24 & 76.79 & 82.17 & 79.38 \\
             \cmidrule{2-7}
             & \multirow{2}{*}{MobileViT-XXS} & L1 pruning & 82.89 & 80.00 & 84.08 & 81.99 \\
             & & DepGraph & 80.83 & 78.40 & 80.89 & 79.62 \\
             \cmidrule{2-7}
...
\begin{table}[t]
    ...
    \resizebox{\columnwidth}{!}{%
        \begin{tabular}{l l l r r r r}
            ...
            % Private Dataset (Updated Values)
            \multirow{11}{*}{\textbf{Private}} 
             & \multirow{2}{*}{EfficientNet-B0} & L1 pruning & 80.24 & 78.85 & 78.34 & 78.59 \\
             & & DepGraph & 81.12 & 79.62 & 79.62 & 79.62 \\
             \cmidrule{2-7}
             & \multirow{2}{*}{MobileNetV4-S} & L1 pruning & 82.30 & 79.04 & 84.08 & 81.48 \\
             & & DepGraph & 81.42 & 83.57 & 74.52 & 78.79 \\
             \cmidrule{2-7}
             & \multirow{2}{*}{Xie2019} & L1 pruning & 82.30 & 81.70 & 79.62 & 80.65 \\
             & & DepGraph & 83.78 & 81.48 & 84.08 & 82.76 \\
             \cmidrule{2-7}
             & \multirow{2}{*}{DeiT-Tiny} & L1 pruning & 53.69 & \phantom{0}0.00 & \phantom{0}0.00 & \phantom{0}0.00 \\
             & & DepGraph & 61.06 & 54.83 & \textbf{90.45} & 68.27 \\
             \cmidrule{2-7}
             & \multirow{2}{*}{MobileViT-XXS} & L1 pruning & 79.35 & 75.74 & 81.53 & 78.53 \\
             & & DepGraph & 78.17 & 77.12 & 75.16 & 76.13 \\
             \cmidrule{2-7}
             & \textbf{Ours} & Original & \textbf{86.73} & \textbf{88.36} & 82.17 & \textbf{85.15} \\
            \bottomrule
        \end{tabular}%
