...
# --- 모델 및 이미지 설정 ---
MODEL_NAME = 'resnet18'
IMG_SIZE = 224  # ResNet은 일반적으로 224x224 이미지를 사용합니다.
IN_CHANNELS = 3
...
class CustomImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            # 임시로 검은색 이미지를 반환하여 훈련이 중단되지 않도록 함
...
def train_model():
    ...
    model.fc = nn.Linear(num_ftrs, NUM_CLASSES) # 마지막 레이어를 클래스 수에 맞게 교체
    model.to(DEVICE)
